...
import re
# Para expresiones regulares (limpieza de texto)
...
def main():
    ...
    **¡Bienvenido al chatbot educativo!** 
    ...
    system_prompt = st.sidebar.text_area(
        "Mensaje del sistema:",
        value="Eres un asistente educativo que debe poder responder preguntas sobre los CVs de distintas personas.",        
        height=100,
    ...
    if user_question and user_question.strip():
        ...
        with st.spinner('🤔 El chatbot está pensando...'):
            try:
                ...
                nombres_posibles = ['Ignacio de Pedro', 'Alejandro Topp', 'Francisco de Pedro', 'Agustina Mermier']

                # Propmt para extraer nombres de la pregunta del usuario
                nombre_prompt_template = """
                Dada la siguiente pregunta, identifica y extrae el nombre y apellido de la persona que pregunta.
                La respuesta tiene que ser solo una instancia de "nombres posibles", sin explicaciones adicionales.
                Por defecto, cuando no se nombra a nadie en la pregunta, utilizar el nombre "Ignacio de Pedro"
                Si no puedes identificar un nombre, responde "desconocido".
                Si se menciona más de uno, devuélvelos separados por una coma.
                
                Ejemplos:
                Pregunta: ¿Qué experiencia tiene María y Juan?
                Maria Garcia, Juan Perez

                Pregunta: ¿Quién es Juan Pérez y David Lopez?
                Juan Perez, David Lopez
                
                """.strip()
                
                nombre_prompt_template_simple = f"""
                {nombre_prompt_template}
                ...
                nombre_prompt = PromptTemplate.from_template(nombre_prompt_template_simple)
                
                # Llamar al modelo Groq para identificar nombres
                identificador_cadena = nombre_prompt | groq_chat | StrOutputParser()
                nombres_identificados_str = identificador_cadena.invoke({
                    "pregunta": user_question,
                })
                
                nombres_identificados = [nombre.strip() for nombre in nombres_identificados_str.split(",")]
                patron_extraccion = r"'(.*?)'"
                nombres_extraidos = re.findall(patron_extraccion, str(nombres_identificados))
                print(f"El nombre extraido es: {nombres_extraidos}")
                # Filtrar los nombres posibles basados en la respuesta del modelo
                nombres_para_filtrar = []
                for nombre_extraido in nombres_extraidos:
                    # Limpiar el texto extraído (eliminar "Nombres posibles:")
                    nombre_limpio = nombre_extraido.replace("Nombres posibles: ", "").strip()
                    
                    # Validar si el nombre limpio está en la lista maestra
                    if nombre_limpio in nombres_posibles:
                        nombres_para_filtrar.append(nombre_limpio)
                                        
                print(f"Nombres para filtrar en Pinecone: {nombres_para_filtrar}")
                
                contexto = ""
                for nombre in nombres_para_filtrar:
                    filtro_metadata = {"nombre": {"$eq": nombre}}
                    retriever=vectorstore.as_retriever(search_kwargs={"filter": filtro_metadata}) 
                    docs = retriever.invoke(user_question)
                    contexto_recuperado = "\n\n".join([doc.page_content for doc in docs])
                    contexto_persona = f"Contexto recuperado de {nombre}: {contexto_recuperado}"
                    contexto += contexto_persona + "\n\n"
                ...
                Responde siempre algo en funcion de la información proporcionada en el contexto de cada persona.
                Si no tienes ninguna informacion, responde "No tengo información sobre esa persona".
